  ...
  <section class="delivery-attempts">
    <div class="list-head">
      ...
      <button class="btn soft" type="button" :disabled="props.loading || props.saving" @click="emit('refresh')">
        {{ props.loading ? 'Refreshing…' : 'Refresh invite attempts' }}
      </button>
    </div>

    <p v-if="props.error" class="error-text">{{ props.error }}</p>
    <p v-else-if="props.loading" class="muted">Loading invite delivery attempts…</p>
    <p v-else-if="!props.rows.length" class="muted">No invite delivery attempts yet.</p>
    ...
    <table v-else class="invite-table attempts-table">
      ...
      <tbody>
        <tr v-for="row in props.rows" :key="row.id">
          ...
          <td>
            <span v-if="row.latest_provider_event">{{ row.latest_provider_event }}</span>
            <span v-else>—</span>
            <span v-if="row.latest_provider_event_at">· {{ formatAttemptTime(row.latest_provider_event_at) }}</span>
          </td>
